...
COMMENT ON COLUMN survey_response.started_at IS '응답 시작일시';
COMMENT ON COLUMN survey_response.submitted_at IS '응답 제출일시';
...
COMMENT ON COLUMN survey_response.deleted_at IS '삭제일시';

CREATE UNIQUE INDEX uq_survey_response_user_survey_completed
ON survey_response(user_id, survey_id)
