...
COMMENT ON TABLE survey_question IS '조사지 문항 테이블';
COMMENT ON COLUMN survey_question.question_id IS 'PK - 문항 UUID';
COMMENT ON COLUMN survey_question.survey_id IS 'FK - 조사지 UUID';
COMMENT ON COLUMN survey_question.section_id IS 'FK - 섹션 UUID';
COMMENT ON COLUMN survey_question.question_sort IS '문항 정렬 순서';
COMMENT ON COLUMN survey_question.question_type IS '문항 유형 - SINGLE_CHOICE, LIKERT';
