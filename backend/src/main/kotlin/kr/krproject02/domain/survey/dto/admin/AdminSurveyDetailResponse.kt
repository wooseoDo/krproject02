...
@Schema(description = "관리자 조사지 상세 응답")
data class AdminSurveyDetailResponse(
    ...
    @field:Schema(description = "이전 버전 조사지 UUID")
    val previousSurveyId: UUID?,

    @field:Schema(description = "조사지 버전")
    val surveyVersion: Int,

    @field:Schema(description = "최신 버전 여부")
    val latest: Boolean,
    ...
    val sections: List<AdminSurveyDetailSectionResponse>,
)

data class AdminSurveyDetailSectionResponse(
    val sectionId: UUID?,
    val sectionSort: Int,
    val title: String,
    val targetAverageScore: BigDecimal?,
    val questions: List<AdminSurveyDetailQuestionResponse>,
)

data class AdminSurveyDetailQuestionResponse(
    val questionId: UUID?,
    val questionSort: Int,
    val questionType: SurveyQuestionType,
    val title: String,
    val score: Int,
    val options: List<AdminSurveyDetailOptionResponse>,
)

data class AdminSurveyDetailOptionResponse(
    val optionId: UUID?,
    val optionSort: Int,
